...
void Test_Det(){
	...
	while(inf>> E0 >> Theta >> q2fm >>  XS_EX >> XS_EX_Err >> B >> A){ 
		...
		vXS_Diff[N] = XS_M/XS_EX;
		vXS_Diff_Err[N] = vXS_Diff[N] * XS_EX_Err/XS_EX;
	...
	c1->Divide(1,2);
	c1->cd(1);
    TH2F *h1 = new TH2F("h1","Elastic Deuteron XS (Simon et.al) vs SOG model ", 100, Q2_Min-1,Q2_Max+1,100,XS_Min-1,XS_Max+2);
	...
	c1->cd(2);
	TH2F *h2 = new TH2F("h2","Elastic Deuteron XS (Simon et.al) vs SOG model ", 100, Q2_Min-1,Q2_Max+1,100,0,2);
	h2->SetXTitle("log_{10}(Q^{2}) (MeV^{2})");
	h2->SetYTitle("(#sigma_{el}^{MC}/#sigma_{el}^{EX}");
    h2->Draw();

	TGraphErrors* gr2= new TGraphErrors(N, Q2_log, vXS_Diff, 0, vXS_Diff_Err);
