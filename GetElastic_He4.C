...
int main(){
	...
	double Theta, Phi, XS;
	double Q2_gen,Theta_gen, Phi_gen,Ep_gen;
    double G_M, G_C;

	TFile *file = new TFile(Form("XS_He4_Elastic_%d_Smear.root",(int) (E0)),"recreate");
	...
	for(int i=0;i<N;i++){
		...
		G_C = GetFF_He4(Q2_gen);

		//Elastic Scattering
		XS = He4_Elastic(E0,Theta_gen);//fm2/sr
		XS *= FM2NB;//nb/sr
